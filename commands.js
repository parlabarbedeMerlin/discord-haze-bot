...
index.bot.on("message", (msg) => {
    ...
    if (content.startsWith("$")) { //exemple 1 de préfix
        check(msg, content, 1); 
    }else if (content.startsWith("ABC")) { //exemple 2 de préfix
        check(msg, content, 1);
    }
});

function check(msg, content, size) {
    //séparation des arguments et supression du préfix
    let commande = content.slice(size);
